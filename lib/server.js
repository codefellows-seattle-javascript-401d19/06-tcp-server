...
let parseCommand = (message,socket) =>{
  if(message.startsWith('@')){
    ...
    case `@nickname`:
      ...
      for(let client of clients){
        if(client.name === newName){
          socket.write(`That name is already taken, please try again` + `\n`);
          return;
        }
      }
    ...
    case `@dm`:
      let targetClient = parsedCommand[1];
      let message = parsedCommand.slice(2).join(' ');
      logger.log(`info`, `${parsedCommand[1]} is the target client`);
      logger.log(`info`, `${parsedCommand[2]} is the message`);

      for(let client of clients){
        logger.log(`info`, `${clients[0].name} is the first client's name`)
        if (client.name === targetClient){
          client.write(`${message}` + `\n`);
          logger.log(`info`, `private message sent: ${message}`);
          socket.write(`Your message was sent to ${targetClient}` + `\n`);
        }
      }
